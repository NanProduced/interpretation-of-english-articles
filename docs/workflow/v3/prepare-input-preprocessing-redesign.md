...
- 对短文本（<50 chars）和混合文本不如 `lingua-language-detector` 稳定

结论：

- 当前实现使用 `langdetect`
- 只作为辅助信号，不单独一票否决
- 如需更高准确率，可切换到 `lingua-language-detector`

来源：

- [langdetect PyPI](https://pypi.org/project/langdetect/)

#### 备选：`lingua-language-detector`
...
spaCy 模型 `en_core_web_sm` 必须显式安装，否则 `spaCy.load()` 抛出 `OSError`，被 `_check_spacy_model()` 捕获后返回 `False`。
...
- 检查：`prepare_input()` 首次调用时触发 `_check_spacy_model()` 检查
- 日志：记录 `WARNING: spaCy model en_core_web_sm unavailable: {OSError}`（不抛出）
- 运行时：强制走 regex 断句，`fast_path=False`
- action 名称：
  - `regex_sentence_split_no_spacy` — spaCy 模型本身不可用（OSError）
  - `regex_sentence_split_spacy_error` — spaCy 运行时崩溃
  - `regex_sentence_split` — 正常兜底（fast_path=False 但 spaCy 可用）
- 结果：降级可观测，不静默
...
- 缩写断句保护：`U.S.` / `U.K.` / `Ph.D.` / `e.g.` / `i.e.` / `Dr.` / `Mr.` / `Mrs.` / `Ms.` / `Prof.` / `Dept.` / `etc.` / `approx.`
- 引号、括号、省略号
- HTML 片段
- Markdown 链接与代码块
- 中英混排长文
- API/字段说明文档
- 极短文本
- spaCy 不可用时的显式降级 action（`regex_sentence_split_no_spacy`）
- 高噪声复制内容
