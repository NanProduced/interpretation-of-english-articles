...
> 归档状态：**已归档，不再作为当前开发参考**
>
> 说明：本文件仅保留为历史背景与复盘材料。当前 workflow 重构、联调、评审一律以 [Workflow V3 设计与重构文档](../../v3/workflow-v3-design.md) 为准；如需理解上一版实现，再参考 [V2.1 改造设计稿](../v2-1-refactor-design.md)。
>
> 更新时间：2026-04-01
> 原状态：前端实验完成，后端 V2 可按本文档进入开发
...
V2 不再继续优化 V1 的业务数组式输出，而是以一套经过前端实验验证的统一渲染模型为核心，让前端、后端和 LLM 围绕同一份协议协作。

---

## 12. 归档总结：V2 失败原因与经验教训

### 12.1 为什么归档

V2 已完成前端实验和一轮后端实现，但最终没有成为主线方案，原因不是“完全不可用”，而是它在核心结构上已经偏离了更适合长期演进的组件化方向。

### 12.2 失败原因

1. 让 LLM 直接输出统一渲染容器，而不是先输出语义组件。
2. 语义类型和 UI 原语耦合过深，导致模型一边做教学判断，一边做渲染分发判断。
3. `inline_marks / sentence_entries / cards` 的统一容器自由度过大，输出稳定性差。
4. `ContextGloss`、多段 `GrammarNote`、结构化 `SentenceAnalysis` 这些真正有价值的能力在统一容器下很难稳定产出。
5. `cards` 过早进入主链路，分散了模型预算和产品焦点。
6. 旧方案里不少字段是“为了前端可渲染”硬塞给 LLM 的，结果让模型更像在填 UI 表单，而不是做教学型结构化标注。

### 12.3 经验教训

1. LLM 应先输出语义组件，再由后端投影成渲染模型。
2. 渲染契约可以统一，但不能直接作为模型原始输出目标。
3. 卡片、百科补充、重型解释能力应后置，先把“帮助读懂文章”做稳。
4. Prompt、schema、validator、projection 必须一起设计，不能只调 prompt。
5. 前端 mock 页面必须完整覆盖所有组件类型，否则很容易在 schema 设计阶段误判。
6. 版本升级时不能长期保留双链路，否则文档和代码很容易漂移。
