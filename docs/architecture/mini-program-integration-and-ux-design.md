...
当前阶段纳入 MVP：真实 /analyze 联调、API adapter、loading/error/empty/degraded 状态、结果页核心交互、历史记录最小闭环、登录策略基础落地、小程序基础平台能力接入。

当前阶段可延后：OCR、订阅消息、学习统计、高级配置中心、复杂推荐与运营能力、每日精读结果页的独立产品形态。

## 11. 核心设计结论

- workflow 继续独立优化，小程序侧同步建立真实接口、真实状态、真实用户路径和真实平台能力
