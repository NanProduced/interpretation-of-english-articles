...
def _build_vocabulary_policy_lines(plan: GoalExecutionPlan) -> tuple[str, ...]:
    ...
    if focus == "high_value_only":
        ...
        return (
            '用户是学术阅读者，他们需要理解论文、学术文献或专业材料。',
            '他们的核心目标是"理解内容"，不是"学英语"。帮助他们快速识别学术结构、理解专业术语、理清论证关系。',
            '标词策略：',
            '1. 优先标注学术术语（如 methodology, paradigm, empirical, theoretical framework）和专业领域词汇，这些是理解的核心障碍。',
            '2. 标注学术高频搭配（如 account for, contribute to, in terms of, with respect to），这些是学术写作的"骨架"。',
            '3. 标注熟词僻义的学术用法（如 address=探讨, school=学派, discipline=学科, novel=新颖的）。',
            '4. 常见词和日常表达不标。',
            '释义策略：',
            '- 术语释义要准确，使用学术领域的标准中文译名。',
            '- phrase_gloss 要说明搭配在学术语境中的功能（如"用于引出原因"、"用于限定范围"）。',
            '- context_gloss 要对比学术义与日常义的差异，帮助用户建立"学术语境敏感度"。',
        )
...
def _build_grammar_policy_lines(plan: GoalExecutionPlan) -> tuple[str, ...]:
    ...
    if focus == "explicit_split":
        ...
        if plan.variant_id == "kaoyan":
            ...
            return (
                '用户是学术阅读者，他们需要快速理解学术文本的句子结构。学术文本的特点是：长句多、被动语态多、名词化表达多、论证结构复杂。',
                '你的目标不是"教语法"，而是"帮用户快速提取信息"——让用户能快速识别：谁做了什么、论点是什么、论证关系如何。',
                'grammar_note 采用"信息定位辅助"方式：',
                '1. 标注影响信息提取的结构：定语从句（限定信息）、被动语态（学术文本极常见，需指出动作执行者或说明"谁在说什么"）、分词结构（压缩信息，需指出修饰对象）、名词性从句（承载核心观点）。',
                '2. 标注逻辑连接结构：让步状语从句（although, however, nevertheless 等，标志论证转折）、原因状语从句（because, since, as 等，标志因果论证）、条件状语从句（if, unless 等，标志假设论证）。',
                '3. 标注指代关系：this, that, these, those, it 等在学术文本中常用来指代前文观点，需帮助用户看清指代什么。',
                'sentence_analysis 是最高优先级——对学术长难句，不仅拆句法，还要分析句子在段落论证中的功能（提出论点/提供证据/反驳观点/过渡衔接）。',
                'chunks 拆到中细粒度，帮助用户看清信息层次。',
...
def _build_translation_policy_lines(plan: GoalExecutionPlan) -> tuple[str, ...]:
    ...
    if style == "literal_support":
        ...
        if plan.variant_id == "kaoyan":
            ...
            return (
                '用户是学术阅读者，翻译服务于快速理解学术内容。',
                '翻译要准确、清晰，保持学术文本的严谨性。',
                '对专业术语要使用标准中文译名，帮助用户建立"学术表达。',
                '长难句的翻译应体现原文的论证结构和逻辑关系，让用户能快速把握论点和论据的关系。',
                '对学术高频搭配和逻辑连接词（如 however, therefore, nevertheless, in contrast），翻译后可补充说明其逻辑功能（如"表示转折"、"引出结论"）。',
                '对指代关系复杂的句子，可在翻译后用括号补充指代说明（如"此处 this 指代前文提出的方法"），帮助用户理清论证脉络。',
            )
        return (
            '翻译追求准确和清晰，适度体现原文结构。',
        )
